...
public:
	...
	void Run()
	{
		...
		script.DeclareParam( L"time", AnimationDataType::Float );
		ExpressionParam* paramTime = script.GetParam( L"time" );
		script.Prepare();
		script.SetParamValue( paramTime, 3.14f );
		Assert( script.GetType() == AnimationDataType::Float );
		...
		Assert( script2.GetType() == AnimationDataType::Float );
		Assert( f == 0.5f );
		...
		script2.DeclareParam( L"time", AnimationDataType::Float );
		...
		Assert( script3.GetType() == AnimationDataType::Color );*/
	}
