...
void GenerateScript( const string& path )
{
	...
	txt.append( "  SetOutPath \"$INSTDIR\\demos\\build\"\n" );
	txt.append( "  File \"..\\..\\demos\\build\\*.slnx\"\n\n" );

	string dmo( path );
	dmo.append( "\\demos\\build" );
	vector<string> demos;
	...
	for ( size_t i = 0; i < demos.size(); ++i )
	{
		...
		txt.append( "  File \"..\\..\\\\build\\" );
		txt.append( demos[i] );
		txt.append( "\\" );
		txt.append( demos[i] );
		txt.append( "_msvs2026.vcxproj\"\n" );

		txt.append( "  File \"..\\..\\demos\\build\\" );
		txt.append( demos[i] );
		txt.append( "\\" );
		txt.append( demos[i] );
		txt.append( "_msvs2026.vcxproj.filters\"\n" );

		txt.append( "  File \"..\\..\\demos\\build\\" );
		txt.append( demos[i] );
		txt.append( "\\" );
		txt.append( demos[i] );
		txt.append( "_msvs2026.vcxproj.user\"\n\n" );
	}

	// Demo binaries.
	txt.append( "  ;Demo binaries\n" );
	txt.append( "  SetOutPath \"$INSTDIR\\\\bin\\win64\"\n" );
	txt.append( "  File \"..\\..\\demos\\bin\\win64\\_DemoConfig.txt\"\n" );

	for ( size_t i = 0; i < demos.size(); ++i )
	{
		txt.append( "  File \"..\\..\\demos\\bin\\win64\\" );
		txt.append( demos[i] );
		txt.append( ".exe\"\n" );
	}
	txt.append( "\n" );
	...
	txt.append( "  SetOutPath \"$INSTDIR\\demos\\src\"\n" );
	txt.append( "  File \"..\\..\\demos\\src\\*.*\"\n\n" );

	txt.append( "  SetOutPath \"$INSTDIR\\demos\\src\\demos\"\n" );
	txt.append( "  File \"..\\..\\demos\\src\\demos\\*.cpp\"\n\n" );

	// Demo media.
	txt.append( "  ;Demo media\n" );

	string md( path );
	md.append( "\\demos\\media" );
	vector<string> media;
	GetDirectories( md, media );
	for ( size_t i = 0; i < media.size(); ++i )
	{
		txt.append( "  SetOutPath \"$INSTDIR\\demos\\media\\" );
		txt.append( media[i] );
		txt.append( "\"\n" );

		txt.append( "  File \"..\\..\\demos\\media\\" );
		txt.append( media[i] );
		txt.append( "\\*.*\"\n\n" );
	}
	...
	{
		string phas( path );
		phas.append( "\\tutorials\\" );
		phas.append( tutorials[i] );
		phas.append( "\\build" );
		vector<string> phases;
		GetDirectories( phas, phases );

		txt.append( "  SetOutPath \"$INSTDIR\\tutorials\\" );
		txt.append( tutorials[i] );
		txt.append( "\\build\"\n" );

		txt.append( "  File \"..\\..\\tutorials\\" );
		txt.append( tutorials[i] );
		txt.append( "\\build\\" );
		txt.append( tutorials[i] );
		txt.append( "_msvs2026.sln\"\n\n" );
	...
	for ( size_t i = 0; i < media.size(); ++i )
	{
		txt.append( "  Delete \"$INSTDIR\\demos\\media\\" );
		txt.append( media[i] );
		txt.append( "\\*.*\"\n" );

		txt.append( "  RMDir \"$INSTDIR\\demos\\media\\" );
		txt.append( media[i] );
		txt.append( "\"\n\n" );
	}
	txt.append( "  RMDir \"$INSTDIR\\demos\\media\"\n\n" );

	txt.append( "  Delete \"$INSTDIR\\demos\\src\\demos\\*.cpp\"\n" );
	txt.append( "  RMDir \"$INSTDIR\\demos\\src\\demos\"\n\n" );

	txt.append( "  Delete \"$INSTDIR\\demos\\src\\*.*\"\n" );
	txt.append( "  RMDir \"$INSTDIR\\demos\\src\"\n\n" );

	txt.append( "  Delete \"$INSTDIR\\demos\\bin\\win64\\*.*\"\n" );
	txt.append( "  RMDir \"$INSTDIR\\demos\\bin\\win64\"\n\n" );

	txt.append( "  RMDir \"$INSTDIR\\demos\\bin\"\n\n" );

	for ( size_t i = 0; i < demos.size(); ++i )
	{
		txt.append( "  Delete \"$INSTDIR\\demos\\build\\" );
		txt.append( demos[i] );
		txt.append( "\\*.*\"\n" );

		txt.append( "  RMDir \"$INSTDIR\\demos\\build\\" );
		txt.append( demos[i] );
		txt.append( "\"\n\n" );
	}

	txt.append( "  Delete \"$INSTDIR\\demos\\build\\*.slnx\"\n" );
	txt.append( "  RMDir \"$INSTDIR\\demos\\build\"\n\n" );

	txt.append( "  RMDir \"$INSTDIR\\demos\"\n\n" );
	...
	for ( size_t i = 0; i < tutorials.size(); ++i )
	{
		...
		txt.append( "  Delete \"$INSTDIR\\tutorials\\" );
		txt.append( tutorials[i] );
		txt.append( "\\build\\*.slnx\"\n\n" );
		...
		for ( size_t p = 0; p < phases.size(); ++p )
		{
			...
			txt.append( "  RMDir \"$INSTDIR\\tutorials\\" );
			txt.append( tutorials[i] );
			txt.append( "\\build\\" );
			txt.append( phases[p] );
			txt.append( "\"\n\n" );
	...
	string dir( path );
	WriteFile( dir.append("\\install\\src\\synkro.nsi"), txt );
}
