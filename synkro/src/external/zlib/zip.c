    ...
    linkedlist_data* ll;    
{
    ...
    while (ldi!=NULL)
    {
        if (ldi->filled_in_this_block > 0)
			if (streamwrite(ldi->data,(uInt)ldi->filled_in_this_block,1,fout)!=1)
                return ZIP_ERRNO;
        ldi = ldi->next_datablock;
    }
    return ZIP_OK;
...
local int ziplocal_putValue OF((void *file, uLong x, int nbByte));
    ...
    int nbByte;
{
    ...
    if (streamwrite(buf,nbByte,1,file)!=1)
        return ZIP_ERRNO;
    else
        return ZIP_OK;
}
...
extern zipFile ZEXPORT zipOpen (stream, append)
    void *stream;
    int append;
{
    zip_internal ziinit;
    ...
    ziinit.filezip = streamopen(stream,(append == 0) ? "wb" : "ab");
    if (ziinit.filezip == NULL)
        return NULL;
    ziinit.begin_pos = streamtell(ziinit.filezip);
    ...
    int level;
{
    ...
    zi->ci.pos_local_header = streamtell(zi->filezip);
    ...
    if ((err==ZIP_OK) && (size_filename>0))
        if (streamwrite(filename,(uInt)size_filename,1,zi->filezip)!=1)
                err = ZIP_ERRNO;

    if ((err==ZIP_OK) && (size_extrafield_local>0))
        if (streamwrite(extrafield_local,(uInt)size_extrafield_local,1,zi->filezip)
                                                                           !=1)
                err = ZIP_ERRNO;
    ...
    unsigned len;
{
    ...
    while ((err==ZIP_OK) && (zi->ci.stream.avail_in>0))
    {
        ...
        {
            if (streamwrite(zi->ci.buffered_data,(uInt)zi->ci.pos_in_buffered_data,1,zi->filezip)
                                                                           !=1)
                err = ZIP_ERRNO;
            zi->ci.pos_in_buffered_data = 0;
            zi->ci.stream.avail_out = (uInt)Z_BUFSIZE;
    ...
    zipFile file;
{
    ...
    if (err==ZIP_OK)
    {
        long cur_pos_inzip = streamtell(zi->filezip);
		if (streamseek(zi->filezip,
    ...
    const char* global_comment;
{
    ...
    centraldir_pos_inzip = streamtell(zi->filezip);
    if (err==ZIP_OK)
    {
        ...
        while (ldi!=NULL)
        {
            if ((err==ZIP_OK) && (ldi->filled_in_this_block>0))
                if (streamwrite(ldi->data,(uInt)ldi->filled_in_this_block,
                                        1,zi->filezip) !=1 )
                    err = ZIP_ERRNO;
    ...
    if ((err==ZIP_OK) && (size_global_comment>0))
        if (streamwrite(global_comment,(uInt)size_global_comment,1,zi->filezip) !=1 )
                err = ZIP_ERRNO;
	streamclose(zi->filezip);
    TRYFREE(zi);
