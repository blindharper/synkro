...
cbuffer cb1 : register( b1 )
{
	...
	float	sp_amountOpacity;		// Opacity map strength [0.0-1.0]
...
Texture2D 	texOpacity;			// Opacity map texture
                                        
//------------------------------------------------------------------------------
// Samplers
//------------------------------------------------------------------------------
SamplerState	samDiffuse;			// Sampler state
SamplerState	samOpacity;			// Sampler state
...
float4 main( Fragment fragment ) : SV_TARGET
{
	...
	color.a = (sp_amountOpacity > 0.0f) ? sp_amountOpacity*texOpacity.Sample(samOpacity, tex).r : fragment.col.a;
	return color;
}
